...
mod wasm {
    ...
    impl AsciiCat {
        ...
        pub fn new(animations: &JsValue) -> Result<AsciiCat, JsValue> {
            ...
            cat_element.set_attribute("style", 
                "position: fixed; \
                 pointer-events: none; \
                 z-index: 99999; \
                 font-size: 3px; \
            ...
            if let Ok(animations_obj) = animations.clone().dyn_into::<js_sys::Object>() {
                ...
                for i in 0..keys.length() {
                    if let Some(key) = keys.get(i).as_string() {
                        if let Ok(value_js) = js_sys::Reflect::get(&animations_obj, &JsValue::from_str(&key)) {
        ...
        fn idle(&mut self) -> Result<(), JsValue> {
            ...
            if self.idle_time > 10 && 
               ...
               self.idle_animation.is_none() {
                ...
                let mut available_animations = vec!["sleep".to_string(), "itch".to_string(), "wash".to_string()];
                ...
                match anim.as_str() {
                    "sleep" => {
                        if self.idle_animation_frame < 8 {
                            ...
                            let frame_name = format!("sleep{}", (self.idle_animation_frame / 4) % 2 + 1);
        ...
        fn update_position(&mut self) -> Result<(), JsValue> {
            self.cat_element.set_attribute("style", &format!(
                "position: fixed; \
                 pointer-events: none; \
                 z-index: 99999; \
                 font-size: 3px; \
