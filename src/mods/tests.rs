...
mod tests {
    ...
    fn test_create_header_box() {
        ...
        assert!(box_content.contains("╔") || box_content.contains("┌") || box_content.contains("+"));
    }
    ...
    fn test_content_manager() {
        ...
        let context = manager.create_page_context().expect("Failed to create context");
    ...
    fn test_box_cache() {
        ...
        let result = cache.get_or_compute("test_key", || Ok("test_value".to_string()));
        assert!(result.is_ok());
